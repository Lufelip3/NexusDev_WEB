...
if (!isset($_GET['notaFiscal_Saida'])) {
    header("Location: ../Venda/index.php");
    exit();
}

$nota = (int)$_GET['notaFiscal_Saida'];

$vendaController = new VendaController();
$venda = $vendaController->localizarVenda($nota);
...
$itens = $itemController->localizarItemVenda($nota);
$totalVenda = $itemController->calcularTotal($nota);
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  ...
  <title>Itens da Venda – NF #<?= $nota ?> | PharmaPulse</title>
  ...
  <aside class="b5-sidebar offcanvas-lg offcanvas-start p-3" tabindex="-1" id="menuLateral" aria-labelledby="menuLateralLabel">
    ...
    <div class="offcanvas-body d-flex flex-column flex-grow-1 p-0">
      ...
      <ul class="nav nav-pills flex-column mb-auto gap-2">
        ...
        <li class="nav-item"><a href="../Venda/index.php" class="nav-link active"><span class="fs-5">📈</span> Vendas</a></li>
      </ul>
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          <h1 class="display-6 fw-bold m-0" style="color:#1a1c4b;">
            Itens da Venda
            <span class="badge ms-2 fw-bold <?= $venda->Finalizada ? 'bg-success' : 'bg-warning text-dark' ?>" style="border-radius:50px;font-size:.6em;">
              <?= $venda->Finalizada ? '✔ Finalizada' : 'Em Aberto' ?>
            </span>
          </h1>
          <p class="text-secondary mb-0">
            NF #<?= $nota ?>
            &nbsp;|&nbsp; Cliente/Drogaria: <strong><?= htmlspecialchars($venda->Nome_Drog ?? 'Venda Direta') ?></strong>
            &nbsp;|&nbsp; Data: <strong><?= $venda->Data_Venda ? date('d/m/Y', strtotime($venda->Data_Venda)) : '—' ?></strong>
          </p>
        </div>
      </div>
      <a href="../Venda/index.php" class="btn btn-outline-secondary fw-bold px-4">← Voltar às Vendas</a>
    </div>
      ...
      <div class="col-sm-4">
        <div class="card border-0 shadow-sm" style="border-left: 4px solid #27ae60 !important;">
          <div class="card-body">
            ...
            <h2 class="fw-bold mb-0 text-success">R$ <?= number_format($totalVenda, 2, ',', '.') ?></h2>
          </div>
        </div>
      </div>
      <div class="col-sm-4">
        <div class="card border-0 shadow-sm" style="border-left: 4px solid #f39c12 !important;">
          <div class="card-body">
            ...
            <h2 class="fw-bold mb-0" style="color:#f39c12;"><?= array_sum(array_column((array)$itens, 'Qtd_ItemVenda')) ?></h2>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="table-responsive">
          <table class="table table-pharma mb-0 align-middle">
            ...
            <tbody>
              ...
              <tr>
                <td class="ps-4 fw-bold text-secondary">#<?= $item->Cod_ItemVenda ?></td>
                <td><span class="badge bg-light text-dark border" style="font-size:.8rem;"><?= htmlspecialchars($item->EAN_Med ?? '—') ?></span></td>
                <td class="fw-bold"><?= htmlspecialchars($item->Nome_Med ?? '—') ?></td>
                <td>
                  <?php
                    $dataVal = $item->DataVal_ItemVenda ?? null;
                    echo $dataVal ? date('d/m/Y', strtotime($dataVal)) : '—';
                  ?>
                </td>
                <td class="text-center">
                  <span class="badge bg-primary px-3 py-2 fs-6"><?= $item->Qtd_ItemVenda ?></span>
                </td>
                <td class="text-end">R$ <?= number_format($item->Valor_ItemVenda, 2, ',', '.') ?></td>
                <td class="text-end pe-4 fw-bold text-success">R$ <?= number_format($item->Qtd_ItemVenda * $item->Valor_ItemVenda, 2, ',', '.') ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
            <tfoot>
              <tr class="table-light">
                ...
                <td class="text-center fw-bold"><?= array_sum(array_column((array)$itens, 'Qtd_ItemVenda')) ?> un.</td>
                <td></td>
                <td class="text-end pe-4 fw-bold text-success fs-5">R$ <?= number_format($totalVenda, 2, ',', '.') ?></td>
              </tr>
            </tfoot>
          </table>
        </div>
